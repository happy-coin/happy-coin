...
#if defined(NDEBUG)
# error "HappyCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc28179a5a89fce7e8c67df9a3aec60ad8fe93c6144c147124938b7824c3544c0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // HappyCoin: starting difficulty is 1 / 2^12
...
{
    // HappyCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // HappyCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // HappyCoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // HappyCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // HappyCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // HappyCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // HappyCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HappyCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HappyCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HappyCoinMiner(CWallet *pwallet)
{
    printf("HappyCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("happycoin-miner");
    ...
    try { loop {
        ...
        printf("Running HappyCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HappyCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HappyCoinMiner, pwallet));
}
